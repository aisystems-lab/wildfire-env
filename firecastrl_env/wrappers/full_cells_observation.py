...
class CellObservationWrapper(ObservationWrapper):
    ...
    DEFAULT_PROPERTIES = ["ignition_time", "spread_rate", "fire_state", "is_river", "helitack_drops", "elevation"]
    ...
    def __init__(
        ...
        if properties is None:
            ...
            if invalid:
                raise ValueError(f"Invalid properties: {invalid}. Available properties: {self.AVAILABLE_PROPERTIES}")
        ...
        self.grid_height = base_env.gridHeight
        self.grid_width = base_env.gridWidth
        ...
        new_spaces["detailed_cells"] = spaces.Box(
            low=0.0, high=1.0, shape=(self.feature_count, self.grid_height, self.grid_width), dtype=np.float32
        )
