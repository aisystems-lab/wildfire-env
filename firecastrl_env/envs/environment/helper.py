...
def populate_grid(width: int, height: int, image: list[list[float]], interpolate: bool = False) -> list[float]:
    ...
    num_grid_cells_per_image_row_pixel = (image_height - 1) / (height - 1) if interpolate else image_height / height
    num_grid_cells_per_image_col_pixel = (image_width - 1) / (width - 1) if interpolate else image_width / width
    ...
    for _r in range(height):
        ...
        for _c in range(width):
            ...
            if interpolate:
                ...
                bottom_right = (
                    image[image_row_index][image_col_index + 1] if image_col_index + 1 < image_width else bottom_left
                )
                top_left = image[image_row_index - 1][image_col_index] if image_row_index - 1 >= 0 else bottom_left
                top_right = (
...
def get_image_data(
    ...
    try:
        ...
        raise RuntimeError(f"Cannot load image {img_src}: {e}") from e

    height, width, _ = raw_data.shape
    result: list[list[int]] = [[map_color(tuple(raw_data[y, x])) for x in range(width)] for y in range(height)]
...
def get_elevation_data(config: dict[str, Any], elevationImage) -> Optional[np.ndarray]:
    """
    ...
    return get_input_data(
        input_data=elevationImage, grid_width=grid_width, grid_height=grid_height, interpolate=True, map_color=height_fn
    )
...
def perform_helitack_array(state: ArrayFireState, array_x: int, array_y: int) -> int:
    ...
    if array_x < 0 or array_x >= grid_width or array_y < 0 or array_y >= grid_height:
        return 0
...
def is_helicopter_on_fire(fire_status_list: np.ndarray, array_x: int, array_y: int) -> bool:
    # Add bounds checking
    if (
        fire_status_list.size == 0
        or array_y < 0
        or array_y >= len(fire_status_list)
        or array_x < 0
        or array_x >= len(fire_status_list[0])
    ):
        print(
            f"Invalid coordinates: x={array_x}, y={array_y}, map size={len(fire_status_list)}x{len(fire_status_list[0]) if fire_status_list else 0}"
        )
        return False
